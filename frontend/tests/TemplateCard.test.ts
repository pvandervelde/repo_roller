...
describe('TemplateCard (CMP-004)', () => {
  ...
  it('fires the onselect callback when the radio input changes', async () => {
    const onselect = vi.fn();
    render(TemplateCard, { props: { ...baseProps, onselect } });
    await fireEvent.change(screen.getByRole('radio'));
