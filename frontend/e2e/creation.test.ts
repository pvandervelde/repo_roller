...
const MOCK_TEMPLATE_DETAIL_WITH_VARS = {
    name: 'data-pipeline',
    description: 'A data pipeline template',
    tags: ['data'],
    repository_type_name: null,
    repository_type_policy: 'optional' as const,
    variables: [
        { name: 'project_name', required: true, default_value: null, description: 'Project name' },
        { name: 'team_name', required: false, default_value: 'engineering', description: null },
    ],
};
...
test.describe('Creation wizard — Step 1: Choose a template (SCR-004)', () => {
    ...
    test('"Next" button is disabled before template selection (UX-ASSERT-006)', async ({
        page,
    }) => {
        await page.route(/\/api\/v1\/orgs\/[^/]*\/templates$/, async (route) => {
            await route.fulfill({
...
test.describe('Repository Created screen (SCR-005)', () => {
    ...
    test('"Create another repository" shown even with invalid param', async ({ page }) => {
        await page.goto('/create/success');
        await expect(
            page.getByRole('link', { name: 'Create another repository' }),
        ).toBeVisible();
    });
});
