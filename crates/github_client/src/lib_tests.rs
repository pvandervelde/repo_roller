...
async fn test_list_webhooks_success() {
    // Given: A repository with 2 webhooks
    // When: list_webhooks is called
    // Then: Should return vector with 2 Webhook structs
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    ...
    assert_eq!(webhooks[0].id, 12345);
    assert_eq!(webhooks[1].id, 67890);
}

#[tokio::test]
async fn test_list_webhooks_empty() {
    // Given: A repository with no webhooks
    // When: list_webhooks is called
    // Then: Should return empty vector (not an error)
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
...
async fn test_create_webhook_success() {
    ...
    // When: create_webhook is called
    // Then: Should successfully create webhook and return Webhook struct
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
...
async fn test_create_webhook_failure() {
    // Given: Invalid webhook configuration (GitHub API returns 422)
    // When: create_webhook is called
...
async fn test_update_webhook_success() {
    // Given: Existing webhook that needs update
    // When: update_webhook is called
    // Then: Should successfully update webhook configuration
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    let webhook_id = 12345;
    let new_url = "https://example.com/webhook-updated";

    Mock::given(method("PATCH"))
        .and(path(format!("/repos/{owner}/{repo}/hooks/{webhook_id}")))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "id": webhook_id,
            "url": new_url,
            "active": true,
    ...
    assert!(result.is_ok());
    let webhook = result.unwrap();
    assert_eq!(webhook.url, new_url);
    assert_eq!(webhook.events.len(), 2);
}

#[tokio::test]
async fn test_update_webhook_not_found() {
    // Given: Non-existent webhook ID
    // When: update_webhook is called
    // Then: Should return error (404)
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    let webhook_id = 99999;
...
async fn test_delete_webhook_success() {
    ...
    // When: delete_webhook is called
    // Then: Should successfully delete webhook (204 No Content)
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    let webhook_id = 12345;
    ...
    let result = client.delete_webhook(owner, repo, webhook_id).await;
...
async fn test_update_label_success() {
    // Given: Existing label that needs update
    // When: update_label is called with new color/description
    // Then: Should successfully update label
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    let label_name = "bug";
...
async fn test_delete_label_success() {
    ...
    // Then: Should successfully delete label (204 No Content)
    let mock_server = MockServer::start().await;
    let owner = "test-owner";
    let repo = "test-repo";
    let label_name = "wontfix";
    ...
    let result = client.delete_label(owner, repo, label_name).await;
