...
pub async fn execute(
    ...
    debug_assert_eq!(
        ...
        planned.len(),
        "BUG: content map is missing entries for some planned files; \
         ensure plan_files and build_content_map enumerate the same file set"
    );
...
/// Formats a plain string as a TOML quoted string value (surrounding
/// double-quotes included), with all special characters properly escaped.
///
/// This prevents TOML injection when user-supplied values such as `--name`
/// or `--author` contain `"` or `\` characters.
...
pub(crate) fn generate_template_toml(name: &str, description: &str, author: &str) -> String {
    let name_v = toml_string_value(name);
    let description_v = toml_string_value(description);
    let author_v = toml_string_value(author);
    format!(
...
pub(crate) fn generate_readme(name: &str, description: &str) -> String {
    format!(
        r#"# {name} (Template)
...
/// at creation time.
pub(crate) fn generate_readme_template(name: &str, description: &str) -> String {
    format!(
        r#"# {{{{repo_name}}}}
...
pub(crate) fn generate_gitignore() -> String {
...
pub(crate) fn generate_gitignore_template() -> String {
...
pub(crate) fn generate_test_template_workflow(name: &str) -> String {
    format!(
...
pub(crate) fn generate_ci_template_workflow() -> String {
...
pub(crate) fn generate_renovate_config() -> String {
    r#"{
