...
proptest! {
    ...
    fn prop_upper_case_is_idempotent(s in "\\PC*") {
        ...
        if let Ok(first) = engine.render_template("{{upper_case v}}", &ctx) {
            let ctx2 = TemplateContext::new(json!({ "v": first.clone() }));
            if let Ok(second) = engine.render_template("{{upper_case v}}", &ctx2) {
                prop_assert_eq!(
    ...
    fn prop_lower_case_is_idempotent(s in "\\PC*") {
        ...
        if let Ok(first) = engine.render_template("{{lower_case v}}", &ctx) {
            let ctx2 = TemplateContext::new(json!({ "v": first.clone() }));
            if let Ok(second) = engine.render_template("{{lower_case v}}", &ctx2) {
                prop_assert_eq!(
                    first.clone(), second.clone(),
    ...
    fn prop_lower_of_upper_equals_lower(s in "[a-zA-Z0-9 _-]{0,64}") {
        ...
        if let (Ok(a), Ok(b)) = (lower_s, lower_of_upper) {
            prop_assert_eq!(
                a.clone(), b.clone(),
